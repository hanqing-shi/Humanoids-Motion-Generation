...
if __name__ == "__main__":
    ...
    parser.add_argument('--folder', type=str, default='./dataset/data_joint/run', help="Input folder containing CSV files")
    parser.add_argument('--feature_out_folder', type=str, default='./dataset/data_feature/run', help="Output folder for processed files")
    parser.add_argument('--label_out_folder', type=str, default='./dataset/data_label/run', help="Output folder for label files")
    ...
    robot = pin.RobotWrapper.BuildFromURDF(
        f'./dataset/g1_retargeted_dataset/{robot_type}/{robot_type}_29dof_rev_1_0.urdf', f'./dataset/g1_retargeted_dataset/{robot_type}', pin.JointModelFreeFlyer()
    )
    ...
    for csv_path in csv_files:
        ...
        feature_out_path = os.path.join(feature_out_folder, f"{file_name}.csv")
    ...
    'right_wrist_yaw_joint'
        ...
        # Use 'enumerate' to get the index 'i' for data, and 'row' for feature_data_out
        feature_data_out = [row[:2] + data[i, 2:].tolist() for i, row in enumerate(label_data_out)]
        ...
        labels_filter = labels_np.copy()

        # Filter parameters.
        iterations = 50  # Total number of filtering passes.
        window_size = 5 # Window size.

        # Apply repeated filtering to each dimension (x, y, z).
        for i in range(iterations):
            for col in range(3):
                labels_filter[:, col] = moving_average_filter(labels_filter[:, col], window_size=window_size)
