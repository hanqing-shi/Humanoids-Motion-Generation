...
def init_joystick():
    if not HAS_PYGAME:
        print("⚠️ pygame not installed; joystick/keyboard control disabled (commands will be zero).")
        return None

    pygame.init()
    pygame.joystick.init()

    # create a small window so pygame can capture keyboard input
    pygame.display.set_caption("G1 Joystick/Keyboard Control")
    ...
    if pygame.joystick.get_count() > 0:
        ...
        print("⚠️ No joystick detected; using keyboard control (W/S = forward/back, A/D = turn).")
...
def read_joystick_command(js):
    ...
    If a joystick is present:
      ...
      - Right stick X (axis 3 or 2): strafe   -> linear_y
      - Left stick X (axis 0): turning        -> angular_z

    If no joystick but pygame is available:
      - W / S : forward / back   -> linear_x
      - A / D : turn left/right  -> angular_z
      - linear_y = 0 for now
    """
    # No pygame at all → zero command
    if not HAS_PYGAME:
