...
def inference_worker(conf, model, device, data_stats, norm_mask, x_init, rerun_urdf):
    ...
    with torch.no_grad():
        for frame in range(inference_steps):
            ...
            with buffer_lock:
                ...
                cmd_history.append(cmd_seq.copy())  # (20, 3) for this frame
    ...
    # Save Results
    if traj_history:
        full_traj = np.concatenate(traj_history, axis=0)   # (N, traj_dim)
        full_cmd  = np.concatenate(cmd_history, axis=0)    # (N, 3)

        os.makedirs(conf['save_dir'], exist_ok=True)

        base = f"{conf['model']['name']}_rt"
        traj_path = os.path.join(conf['save_dir'], f"{base}_traj.csv")
        cmd_path  = os.path.join(conf['save_dir'], f"{base}_cmd.csv")

        np.savetxt(traj_path, full_traj, delimiter=",")
        np.savetxt(cmd_path,  full_cmd,  delimiter=",")

        print(f"✅ Saved traj to {traj_path}")
        print(f"✅ Saved cmd  to {cmd_path}")
